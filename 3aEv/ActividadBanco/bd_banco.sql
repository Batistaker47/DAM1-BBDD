...
CREATE DEFINER=`root`@`localhost` PROCEDURE `cta_modificar` (IN `_nif` VARCHAR(9), IN `_titular` VARCHAR(50), IN `_id` INT)   begin
		UPDATE cuentas SET
        c_nif = _nif,
        c_titular = _titular
        WHERE 
...
--
ALTER TABLE `cuentas`
  MODIFY `c_num_cta` int(10) UNSIGNED ZEROFILL NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=13;
